...
- [x] **Arrays & Lists:** Создание списков `[1, 2, 3]`, индексация `arr[0]` и трансляция в `Vec<T>`.
- [x] **Dictionaries:** Базовая поддержка пар ключ-значение (трансляция в `HashMap`).
- [ ] **String Manipulation:** Встроенные методы для строк (конкатенация, интерполяция).

### `v0.4.0` — Architecture & OOP (Архитектура и ООП)
- [x] **Object Model:** Реализация `class` с прозрачной авто-генерацией `struct` и `impl` в Rust.
...
- [x] **CLI Tool:** Полноценное приложение `crest` (команды `init`, `run`, `build`).
